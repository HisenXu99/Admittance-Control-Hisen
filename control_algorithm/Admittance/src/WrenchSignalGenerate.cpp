...
int main(int argc, char ** argv)
{
    ...
    ros::Publisher wrench_pub = nh.advertise<geometry_msgs::WrenchStamped>(WRENCH_TOPIC, 5);   //WRENCH_TOPIC为话题，上面定义了宏，实际为/wrench
    ros::Duration Sleep(5.0);
    Sleep.sleep();
    ros::Rate   loop_rate(TOPIC_HZ);
    geometry_msgs::WrenchStamped wrench_msg;    //六维力数据结构
    double t = 0;
    while (ros::ok())
    {
