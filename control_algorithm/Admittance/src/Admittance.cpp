...
Admittance::Admittance(ros::NodeHandle &n,
  ...
  while (nh_.ok() && !arm_position_(0)) {     //等待机械臂回到原位，但是我在docker中机械臂一致没办法回到原位,gazebo版本太旧。
...
void Admittance::run() {
  ...
  while (nh_.ok()) {
    ...
    ros::spinOnce();      //spinOnce调用回调函数队列中第一个callback函数
    loop_rate_.sleep();   //配合执行频率，sleep一段时间，然后进入下一个循环。
  }
}
...
void Admittance::compute_admittance() {
  ...
  error.topRows(3) = arm_position_ - desired_pose_position_;                        //计算xyz的
  ...
  error.bottomRows(3) << err_arm_des_orient.axis() * err_arm_des_orient.angle();        //上面几行全是计算roation的
